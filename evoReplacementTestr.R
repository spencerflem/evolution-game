  ...
  output$evos <- renderMenu({
    sidebarMenu(
      menuItem("body",tabName = "body",
               menuSubItem("larger (+1 size)",tabName = "+1size"),
               menuSubItem("smaller (-1 size)",tabName = "-1size")),
      menuItem("legs",tabName = "legs",
               menuSubItem("speedier (+1 speed)",tabName = "+1speed"),
               menuSubItem("more efficent (-1 speed)",tabName = "-1speed")),
      menuItem("hide",tabName = "hide",
               menuSubItem("defensive (+1 def, -1 camo)",tabName = "+1def,-1camo"),
               menuSubItem("camouflaged (+1 camo -1 def)", tabName = "+1camo,-1def")),
      menuItem("eyes",tabName = "eyes",
               menuSubItem("longer ranged (+2 sight)",tabName = "+2sight"),
               menuSubItem("camo spotting (+2 spot)",tabName = "+2spot"),
               menuSubItem("more efficent (-1 sight, -1 spot)",tabName="-1sight,-1spot")),
      menuItem("stomach", tabName = "stomach",
               menuSubItem("plant digesting", tabName = "????"),
               menuSubItem("meat digesting", tabName = "???"),
               menuSubItem("poison resisting", tabName = "??")),
      menuItem("glands", tabName = "glands",
               menuSubItem("deadlier (+1 poison)", tabName = "+1poison"),
               menuSubItem("more efficent (-1 poison)", tabName = "-1poison")),
...
baseCreature <- data.frame(size = 5, speed = 5, def = 5, camo = 5, sight = 5, spot = 5, min = 5, meat = 5, resist = 5, poison = 5, flight = 5, pack = 5, grass = FALSE)
creatures <- list('90' = baseCreature)

evolution <- "-1/sight,-1/spot"
items <- strsplit(evolution, ",")
amounts <- strsplit(unlist(items), "/")
dfAmounts <- as.data.frame(amounts, stringsAsFactors = FALSE)
stats <- dfAmounts[2,]
values <- sapply(dfAmounts[1,], as.numeric)
length(stats)
creatures[['90']][[stats[[1]]]] <- creatures[['90']][[stats[[1]]]] + values[[1]]

value <- unlist(amounts)[1]
as.numeric(value)
